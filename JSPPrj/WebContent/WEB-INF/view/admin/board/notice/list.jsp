			...
			<main class="main">
				...
				<form action="list" method="POST">
					<div class="notice margin-top">
						...
						<c:forEach var="notice" items="${noticeList }" begin="0" end="4" varStatus="st">
						
						<tr>
							<td>${notice.id }</td>
							<td class="title indent text-align-left">
							<a href="detail?id=${notice.id }">
							${notice.title }</a> <span>[${notice.cmtCount }]</span> </td>
							<td>
							${notice.writerId }
							</td>
							<td>
							...
							<c:set var="checked" value="${notice.pub eq 'true'? 'checked':'' }"/>
							<td><input type="checkbox" ${checked } name="open-id" value="${notice.id }"></td>
				...
				<div class="margin-top align-center pager">
					...
					<c:if test="${startNum>1 }">
							<a class="btn btn-next" href="?p=${startNum-1 }&f=${param.f }&q=${param.q}">이전</a>
						</c:if>
						<c:if test="${startNum<=1   }">
					...
					<ul class="-list- center">
						...
						<c:if test="${(startNum+i) <= lastNum }">
							<li><a class="-text- ${(page==(startNum+i))?'orange bold':'' }" href="?p=${startNum+i }&f=${param.f }&q=${param.q}" >
							${startNum+i }
							</a></li>
						</c:if>
						</c:forEach>	
					...
					<div>
						...
						<c:if test="${startNum+4>= lastNum  }">
							<span class="btn btn-next" onclick="alert('다음 페이지가 없습니다.');">이전</span>
						</c:if>
					</div>
				</div>
			</main>
